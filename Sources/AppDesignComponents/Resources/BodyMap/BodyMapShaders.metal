...
fragment float4 bodyMapFragment(
    ...
    const float2 sampleUV = frame.metadata.z > 0.5
        ? bodyMapMorphologySourceUV(
            uv,
            frame.torsoScales,
            frame.limbScales
        )
    ...
    const float baseMask = masks.sample(bodyMapSampler, sampleUV, 0).r;
